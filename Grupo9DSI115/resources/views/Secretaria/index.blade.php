        ...
        <div class="card-header">
            <div class="row">
                <div class="col-md-12 col-12 d-flex justify-content-center align-items-center">
                    <form class="w-100 d-flex justify-content-center align-items-center" method="GET"
                        action="{{ route('dshSecretaria.index') }}">
                        <div class="container">
                            <div class="row">
            ...
            <div class="row">
                <div class="col-md-12 col-12 p-1 d-flex justify-content-center align-items-end">
                    <a class="btn btn-primary" id="mediumButton" href="#" role="button" data-toggle="modal"
                        data-target="#mediumModal" data-attr="{{ route('citas.create') }}">Crear Cita</a>
                </div>
            </div>
        </div>
        <div class="card-body">
            @if ($fechaInicio && $fechaFin)
                <h2 class="text-center">Citas programadas entre las fechas: {{ $fechaInicio }} - {{ $fechaFin }}
                </h2>
                @if (count($citas) <= 0)
                    <h2 class="text-center">No hay citas programadas</h2>
                @endif
            @elseif (count($citas)<=0) <h2 class="text-center">No hay citas programadas</h2>
                @else
                        ...
                        <div class="bg-dark2 rounded mb-4">
                            <div class="card-header d-flex justify-content-between align-items-center" id="headingOne">
                                <h3 class="mb-0 d-block">
                                    <a class="btn btn-link text-white" id="mediumButton" href="#" role="button"
                                        .createdata-toggle="modal" data-target="#mediumModal"
                                        data-attr="{{ route('consultasByDashboard.create', $cita) }}">
                                ...
                                <button class="btn btn-link text-left collapsed text-white-50" type="button"
                                    data-toggle="collapse" data-target="#collapse{{ $cita->id }}" aria-expanded="false"
                                    aria-controls="collapse{{ $cita->id }}">
                                    <i class="fas fa-arrow-down text-white"></i>
                                </button>

                            </div>
                            <div id="collapse{{ $cita->id }}" class="collapse" aria-labelledby="headingOne"
